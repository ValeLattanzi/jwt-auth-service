...
public class AuthHandler : AuthenticationHandler<AuthenticationSchemeOptions> {
	...
	protected override Task<AuthenticateResult> HandleAuthenticateAsync() {
		return Task.Run(validateCookies);
		...
		// var jwtRefreshToken = _refreshToken[$"{AUTH_SCHEME} ".Length..].Trim();
		//
	...
	private static List<Claim> getClaimsFromToken(string jwtToken, JwtSecurityTokenHandler tokenHandler, byte[] key) {
		try {
			tokenHandler.ValidateToken(jwtToken, new() {
				ValidateIssuerSigningKey = true,
				IssuerSigningKey = new SymmetricSecurityKey(key),
	...
	private AuthenticateResult validateCookies() {
		if (!Request.Cookies.ContainsKey("access-token") || !Request.Cookies.ContainsKey("refresh-token"))
			return AuthenticateResult.Fail("Unauthorized");

		var _accessToken = Request.Cookies["access-token"];
		var _refreshToken = Request.Cookies["refresh-token"];

		// Validate if the access token is valid and if the refresh token is present
		if (string.IsNullOrWhiteSpace(_accessToken) ||
		    !_accessToken.StartsWith(AUTH_SCHEME) ||
		    string.IsNullOrWhiteSpace(_refreshToken))
		...
		_accessResult = validateToken(_newAccessToken, _tokenHandler, _accessKey);
		if (!_accessResult.Succeeded) return AuthenticateResult.Fail("Invalid access token");

		Response.Cookies.Append("Authorization", $"{AUTH_SCHEME} {_newAccessToken}", new() { HttpOnly = true });
		return _accessResult;
	}
}